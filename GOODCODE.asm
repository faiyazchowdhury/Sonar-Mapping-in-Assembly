...
X1:        DW &B0001000100010001
X2:        DW &B0010001000100010
X4:        DW &B0100010001000100
X5:        DW &B1000100010001000
Y1:        DW &B0000000000001111
Y2:        DW &B0000000011110000
...
StoreLeftLoc1:  ;Is called while bot is in coordinate (3,1)
	CALL   SETX0
	LOAD   LEFT
	SUB    MIDDIST   ;Is the distance reading between the bot and the wall?
	JNEG   SetXLeft ;If so, set the x value of the object's location coordinate.
	RETURN
			
StoreRightLoc1:  ;Is called while bot is in coordinate (3,1)
	CALL   SETX6
	LOAD   RIGHT
	SUB    MIDDIST   ;Is the distance reading between the bot and the wall?
	JNEG   SetXRight ;If so, set the x value of the object's location coordinate.
	RETURN
		
StoreLeftLoc2:   ;Is called while bot is in coordinate (3,2)
	CALL   SETX0	    ;Sets Y value of coordinate to 2
	LOAD   LEFT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXLeft		;If so, set the x value of the object's location coordinate.
	RETURN
		
StoreRightLoc2:  ;Is called while bot is in coordinate (3,2)
	CALL   SETX6		;Sets Y value of coordinate to 2
	LOAD   RIGHT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXRight	;If so, set the x value of the object's location coordinate.
	RETURN
		
StoreLeftLoc3:  ;Is called while bot is in coordinate (3,3)
	CALL   SETX0		;Sets Y value of coordinate to 3
	LOAD   LEFT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXLeft		;If so, set the x value of the object's location coordinate.
	RETURN
		
StoreRightLoc3: ;Is called while bot is in coordinate (3,3)
	CALL   SETX6			;Sets Y value of coordinate to 3
	LOAD   RIGHT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXRight	;If so, set the x value of the object's location coordinate.
	RETURN
		
StoreLeftLoc4: ;Is called while bot is in coordinate (3,4)
	CALL   SETX0
	LOAD   LEFT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXLeft		;If so, set the x value of the object's location coordinate.
...
StoreRightLoc4: ;Is called while bot is in coordinate (3,4)
	CALL   SETX6
	LOAD   RIGHT
	SUB    MIDDIST		;Is the distance reading between the bot and the wall?
	JNEG   SetXRight	;If so, set the x value of the object's location coordinate.
...
SetXLeft:  ;Extrapolates and sets the X coordinate based on the left side sensor reading and the known odometer reading on the bot.	
	LOAD   LEFT
	SUB    CLOSEDIST  	;Is the object in column 1?
	JNEG   SetX2	;If not set X to 1.
	JUMP   SetX1	;If so set X to 2.
		
		
SetXRight:  ;Extrapolates and sets the X coordinate based on the right side sensor reading and the known odometer reading on the bot.
	LOAD   RIGHT
	SUB    CLOSEDIST	;Is the object in column 5?
	JNEG   SetX4	;If not set X to 4.
	JUMP   SetX5	;If so set X to 5.
	;
...
; THE FOLLOWING IS A CALL TO STORE VALUES IN A MATRIX

STORELOCMAT:    ;;;; VERY USEFUL CODE
	LOAD  ROWMAT
	AND   COLMAT
	OR    LOCMAT
	STORE LOCMAT
...
RETURNSTATE:
	...
	;STORE  LASTPOS
	RETURN
READBOTH: ;Reads sensors 0 and 5.
	IN     DIST0
	STORE  LEFT
	OUT    SSEG1
	IN     DIST5
...
SendObjCount:
	CALL   UARTNL      ; send a newline
	LOAD   AsciiN      ; send ascii 'X'
	CALL   UARTRAW
	LOAD   ObjCount    ; send X coordinate
	CALL   UARTSEND1
	RETURN
