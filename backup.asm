...
Main: ; "Real" program starts here.
	...
	; INITALIZING VALUES
	LOAD   MASK0     ;ENABLE SONARSENSORS
	OR     MASK5
	OUT    SonarEN
	LOAD   Zero      ;SET VARIABLES
	STORE  ObjCount
	STORE  ObjX
	STORE  ObjY
	STORE  LEFT
	...
	LOAD   Fmid
	STORE  DesVel
	LOAD   EMPTYMAT  ;MATRIX VARIABLES
	STORE  ROWMAT
	STORE  COLMAT
	STORE  LOCMAT
	;
...
;LRY3:
	...
	CALL   STORELOCMAT
;	JUMP   CHECKY3
;CHECKY3:  ;IF ROW 3 IS EMPTY WON'T BOTHER TO CHECK TOP ROW
	LOAD   LOCMAT
	AND    Y3
	JZERO  SCANY3
	JUMP   U33
...
LRY4:
	...
	CALL   StoreRight
	CALL   STORELOCMAT
		OUT    RESETPOS   ; FOR REMAINING SCANS
		LOAD   Zero       ;
		STORE  LASTTHETA  ;
		STORE  LASTPOS    ;
	CALL   CALCOBJCOUNT
...
; object reporting.  At the end of the run, the robot will send an object count
; of 0, 1, or 2, depending on whether neither, one, or both sensors saw
; nearby objects.

; This example uses a movement helper subroutine based on the last two semester
; projects (which investigated ways to make the robot move accurately).
...
Ex1TestDist:
	...
	SUB    TwoFeet
	SUB    TwoFeet
	SUB    TwoFeet      ; lazy six feet
	JNEG   Ex1Move      ; not there yet; keep moving
	
; at this point, robot has passed 6ft
	LOAD   LeftFlag
	ADD    RightFlag
...
; the truncation of the division (i.e. remainder discarded).
	...
	STORE  ObjX        ; this will be sent as the object's X
	...
	CALL   Div16s      ; 16-bit signed division subroutine
	LOAD   dres16sQ    ; quotient result
	STORE  ObjY        ; this will be sent as the object's Y
	
	CALL   SendObjCoord ; subroutine to send coordinates
	LOADI  5
	OUT    BEEP        ; user feedback that something happened
	LOADI  3
	CALL   WaitAC      ; pause for 0.3s for UART rate limiting
	LOADI  0
...
; Movement helper subroutine.  It sets the wheel velocities based
; on DesTheta and DesVel in order to try to maintain the heading.
DesTheta: DW 0
DesVel:   DW 350
	; OUR CONSTANTS
EMPTYMAT:  DW &B0000000000000000
X1:        DW &B0001000100010001 ;      5___________
X2:        DW &B0010001000100010 ;      4|C|_| |_|F|   Number in grid represents the position of a 1 on the 16 bit binary
X4:        DW &B0100010001000100 ; Y =  3|8|_| |_|_|
X5:        DW &B1000100010001000 ;      2|4|_| |_|_|
Y1:        DW &B0000000000001111 ;      1|0|1| |2|3|
Y2:        DW &B0000000011110000 ; X =  0 1 2 3 4 5 6
Y3:        DW &B0000111100000000
Y4:        DW &B1111000000000000
FARDIST:   DW 1743
MIDDIST:   DW 1162
CLOSEDIST: DW 581
FourFeet:  DW 1162
...
TenFeet:   DW 2905
	;
	;OUR VARIABLES
LEFT:      DW 0  ;STORED DIST READINGS
RIGHT:     DW 0
LASTPOS:   DW 0
LASTTHETA: DW 0
LOCMAT:    DW &B0000000000000000
ROWMAT:    DW &B0000000000000000
COLMAT:    DW &B0000000000000000
	;
...
; back to the desired heading.
	...
	;FOLLOWING ARE CALLS OF OBJECT LOCATION
; FOR SETTING X
StoreLeft:
	LOAD   LEFT
	SUB    CLOSEDIST 
...
StoreRight:
	...
	JUMP   SetX6
	;
;  FOR SETTING Y. IN HERE RIGHT IS UP, AND LEFT IS DOWN.
STOREUP3:
	LOAD   RIGHT
	SUB    CLOSEDIST
	JNEG   SetY4
...
;  SETTING X AND Y VALUES. ALSO SETS ROWMAT AND COLMAT, WHICH ARE LATER USED FOR LOCMAT
...
SetX1: ;Leftmost column inside grid
	LOAD  One
	STORE ObjX
	LOAD  X1
	STORE ROWMAT
...
SetX5: ;Rightmost column inside grid
	LOAD  Five
	STORE ObjX
	LOAD  X5
	STORE ROWMAT
...
SetY5:
	...
	; FOR STORING VALUES IN A MATRIX
STORELOCMAT:
	LOAD  ROWMAT
	AND   COLMAT
	JZERO RETURNSTATE ;IF THE OBJECT IS OUTSIDE THE GRID, EXIT
	OR    LOCMAT
	STORE LOCMAT
	;Load ObjCount
	;ADDI 1
	...
	RETURN	
	;
	; FOR CALCULATING THE OBJECT COUNT FROM LOCMAT
CHECKOBJCOUNT:   ; Calculates the object count. Stops and sends object count if there are 6 objects, and then dies
	LOAD  ObjCount
	SUB   Six
	JNEG  RETURNSTATE
	LOAD  Six ; JUST IN CASE WE CALCULATED MORE THAN 6 OBJECTS BY ACCIDENT.
	STORE ObjCount
	CALL  Wait1
	CALL  SendObjCount
	CALL  Wait1
	JUMP  STOPLOOP
CALCOBJCOUNT:   ; Calculates the object count.
	LOAD  Zero
	STORE ObjCount
	JUMP  COUNT16
	COUNT16:
		LOAD  LOCMAT
		JZERO RETURNSTATE
		JPOS  COUNT15     ; ELSE IT IS NEGATIVE (MSB IS 1)
		LOAD  ObjCount
		ADD   One
		STORE ObjCount
		JUMP  COUNT15
	...
	; FOR MOVEMENT
FORWARD2FEET: ; Moves 2 feet forward, but requires that the bot know LASTPOS correctly.
	;OUT    RESETPOS
	JUMP   FORWARDLOOP
	FORWARDLOOP:
		CALL   FORWARD
...
BACKWARD2FEET: ; Moves 2 feet backward, but requires that the bot know LASTPOS correctly.
	;OUT    RESETPOS
	JUMP   BACKWARDLOOP2
	BACKWARDLOOP2:
		CALL   BACKWARD
...
BACKWARD4FEET: ; Moves 4 feet backward, but requires that the bot know LASTPOS correctly.
	;OUT    RESETPOS
	JUMP   BACKWARDLOOP4
	BACKWARDLOOP4:
		CALL   BACKWARD
...
BACKWARD6FEET: ; Moves 6 feet backrward, but requires that the bot know LASTPOS correctly.
	;OUT    RESETPOS
	JUMP   BACKWARDLOOP6
	BACKWARDLOOP6:
		CALL   BACKWARD
...
TURNLEFT: ; Turns left 90 degrees.
	...
	LEFTLOOP:
		...
		CALL   READBOTH
		IN     Theta
		SUB    Deg270 ;UPPERBOUND
		JPOS   LEFTLOOP
		IN     Theta
		SUB    Deg90  ;LOWER BOUND
		JNEG   LEFTLOOP
		JUMP   RETURNSTATE
		
TURNRIGHT: ; Turns right 90 degrees.
	JUMP   RIGHTLOOP
	RIGHTLOOP: ; THETA MUCH STOP BETWEEN 90 AND 270
		CALL   RIGHTING
		CALL   READBOTH
		IN     Theta
		SUB    Deg90  ;LOWER BOUND
...
RETURNSTATE:   ; A return for JPOS,JNEG and JZERO
	RETURN
READBOTH: ;Reads sensors 0 and 5. Stores values into LEFT and RIGHT, respectively
	IN     DIST0
	STORE  LEFT
	OUT    SSEG1
	IN     DIST5
	...
	OUT    SSEG2
	RETURN
FORWARD: ;Moves bot straight forward
	LOAD   Zero
	STORE  DesTheta    ; desired heading (0 degrees)
	LOAD   FMid
	STORE  DesVel      ; desired velocity (medium forward)
	CALL   SetMovement
	RETURN
BACKWARD: ;Moves bot straight backward.
	LOAD   Zero
	STORE  DesTheta    ; desired heading (0 degrees)
	LOAD   RMid
	STORE  DesVel      ; desired velocity (medium forward)
...
; Before calling, store X and Y coordinates in these variables:
ObjX: DW 0
ObjY: DW 0
SendObjCoord:
